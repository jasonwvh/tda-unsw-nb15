...
class EulerCharacteristicProfile(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if not self.val_ranges:
            if self.pt_cld:
                val_quantiles = [(min([np.quantile(vec_st[:, 1][vec_st[:, 1] > 0], self.quantiles[0][0]) for vec_st in
                                       vec_sts]),
                                  max([np.quantile(vec_st[:, 1][vec_st[:, 1] > 0], self.quantiles[0][1]) for vec_st in
                                       vec_sts]))] + [(min([np.quantile(vec_st[:np.argmax(vec_st[:, 0] < 0), i + 1],
                                                                        self.quantiles[i][0]) for vec_st in vec_sts]),
                                                       max([np.quantile(vec_st[:np.argmax(vec_st[:, 0] < 0), i + 1],
                                                                        self.quantiles[i][1]) for vec_st in vec_sts]))
                                                      for i in range(1, self.n_params_)]
            else:
                val_quantiles = [(min([np.quantile(vec_st[:, i + 1], self.quantiles[i][0]) for vec_st in vec_sts]),
                                  max([np.quantile(vec_st[:, i + 1], self.quantiles[i][1]) for vec_st in vec_sts])) for
                                 i in range(self.n_params_)]
...
class RadonTransform(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, resolution=(), quantiles=[], val_ranges=[], flatten=True, pt_cld=False, normalize=True,
                 stretch_factor=4., min_val_quantile=1e-7):
        self.resolution = resolution
        self.quantiles = quantiles
        self.val_ranges = val_ranges
        self.flatten = flatten
    ...
    def fit(self, X, y=None):
        ...
        if not self.val_ranges:
            if self.pt_cld:
                val_quantiles = [min([np.quantile(vec_st[:, 1][vec_st[:, 1] > 0], self.quantiles[0]) for vec_st in
                                      vec_sts])] + [
                                    min([np.quantile(vec_st[:np.argmax(vec_st[:, 0] < 0), i + 1], self.quantiles[i]) for
                                         vec_st in vec_sts]) for i in range(1, self.n_params_)]
            else:
                val_quantiles = [min([np.quantile(vec_st[:, i + 1], self.quantiles[i]) for vec_st in vec_sts]) for i in
                                 range(self.n_params_)]
            self.val_quantiles_ = val_quantiles
            self.val_ranges = [(0, self.stretch_factor / max(self.min_val_quantile, val_q)) for val_q in
                               self.val_quantiles_]
        return self
...
class HybridTransform(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, resolution=(), quantiles=[], val_ranges=[], kernel_name=None, kernel=None, stretch_factor=4,
                 min_val_quantile=1e-7, flatten=True, pt_cld=False, normalize=True):
        self.resolution = resolution
        self.quantiles = quantiles
        self.val_ranges = val_ranges
    ...
    def transform(self, X):
        ...
        if self.kernel is None:
            ...
            bin_sizes = [(self.val_ranges[i][0], self.val_ranges[i][1], self.resolution[i]) for i in
                         range(self.n_params_)]
            hts = []
            for vec_st in vec_sts:
                if self.pt_cld and self.normalize:

...
from .utils import compute_OR_curvature, compute_FR_curvature, compute_hks_signature, compute_closeness_centrality, \
...
def gen_torus_unif(n_pts, noise, a, c):
    ...
    while n_filled < n_pts:
        ...
        if eta < fx:
            ...
            X[n_filled] = [(c + a * np.cos(theta)) * np.cos(phi), (c + a * np.cos(theta)) * np.sin(phi),
                           a * np.sin(theta)]
            n_filled += 1
...
def build_vectorized_st_from_adjacency_matrix(A, filtrations):
    ...
    for idx, x in enumerate(xs):
        ...
        vec_st[id_edge, 0] = -1
        for idf, f in enumerate(filtrations):
            vec_st[id_edge, idf + 1] = np.max(
                np.take(f, [x, ys[idx]]))  # Compute the upper star filtration on vertices of the edge
    return vec_st
...
kernels = {
    ...
    'weighted': Kernel(lambda i, r: 3 * (
                np.power(r, i + 1) - 2 * np.power(r, i + 2) * (i + 1) / (i + 2) + np.power(r, i + 3) * (i + 1) / (
                    i + 3)),
                       lambda i, r: np.power(r, i) * np.sqrt(i + 1)),
...
def _build_print_point(output, quiet, n):
    ...
    if output is None and quiet:
        ...
        print_point_txt = lambda x, y, i: _sys.stdout.write(message.format(i + 1)) and output.write(
            "{0} {1}\n".format(x, y))
